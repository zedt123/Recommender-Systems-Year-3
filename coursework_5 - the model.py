...
class DataIterableDataset(IterableDataset):
    ...
    def line_mapper(self,line):
        ...
        target = array[25].astype('float32') # 25 is target
...
class WDModel(nn.Module):
    def __init__(self,n_features=88,n_movies = 3000,max_hist_movies = 2834,n_embedding=16):
        ...
        self.out = nn.Linear(n_movies + 256,n_movies)
    ...
    def forward(self,features,h_sparse,h_idx):
        ...
        x = self.out(x) # shape: [batch_size,n_movies]
        return x
...
def h_process(history, n_movies = 3000,max_hist_movies = 2834):
    ...
    for h in history:
        ...
        h_sparse[h] = 1
        h_sparse = torch.tensor([h_sparse])

        h_idx = np.pad(h,(0, (max_hist_movies - len(h))), 'constant')
...
# """
if device == 'cpu':
    params = torch.load('wnd_1.chkpt',map_location=torch.device('cpu'))
else:
    params = torch.load('wnd_1.chkpt')
...
# parameters in the model: (using sum(p.numel() for p in model.parameters() if p.requires_grad))
# for all movies 719338936
# for 10000 movies 103496720
# for 10000 movies 103496720
# for 5000 movies 27128648
# for 3000 movies 10582648
# for 1000 movies 10582648

# uncomment below to save the model
# torch.save({'model':model.state_dict(), 'optimiser':optimizer.state_dict(), 'epoch':epoch}, 'wnd_1.chkpt')
...
with torch.inference_mode():
    for i, batch in enumerate(test_iterable):
        ...
        x = model(features,h_sparse_tensor,h_idx_tensor) # there are the logits, shape:[batch_size,n_movies]
        probs = torch.nn.functional.softmax(x) # turn logits to probabilities     
 
        
        # to get the rating of the movies we want
        # these are the movies we want to find ratings for
        testing_movies = torch.from_numpy(ratings_test['movieId'].iloc[i*batch_size:(i+1)*batch_size].to_numpy()) # shape: [batch_size]

        for j,movie in enumerate(testing_movies):
            prob = probs[j,movie.long()].cpu().item() # this is the probability we have for each movie in the testing data
            # add all ratings, as probabilities and then renormalise them to ratings
...
# we do this by considering the distribution of the ratings, which is almost a normal distribution with mean 3.5 and variance 1.0
predictions = np.array(predictions)
m = np.mean(predictions)
sd = np.std(predictions)
norm_pred = (predictions-m)/sd + 3.5
norm_pred = np.clip(norm_pred,a_min=0.5,a_max=5.0)

ratings_test['predictions'] = norm_pred
